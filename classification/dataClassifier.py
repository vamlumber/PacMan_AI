...
def enhancedPacmanFeatures(state, action):
    ...
    nearest_ghosts = []
    for i in ghosts:
        nearest_ghosts.append(util.manhattanDistance(pac_pos, i))
    ...
    nearest_caps = []
    for j in capsules:
        nearest_caps.append(util.manhattanDistance(pac_pos, j))
    ...
    nearest_food=[]
    for k in food:
        nearest_food.append(util.manhattanDistance(pac_pos, k))
    nearest_food = sorted(nearest_food) 
    for i, weight in zip(xrange(min(len(nearest_food), 5)), [1.2, 0.8] + [0.9] * 3):
