...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: 
            ...
            for locate in self.legalPositions:
                distance = util.manhattanDistance(locate, pacmanPosition)
                allPossible[locate] = emissionModel[distance]*self.beliefs[locate]
        "*** END YOUR CODE HERE ***"
        ...
        if noisyDistance is None:
            ...
            for pos in self.legalPositions:
                ...
                if emissionModel[dist] > 0:
                    new_belief_dist[pos] = float(emissionModel[dist]) * float(belief_dist[pos])
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        pos_positions = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(pos_positions)

        sum1 = 0
        self.particles = []

        while sum1 < self.numParticles:
            for position in pos_positions:
                if sum1 < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        for index, particle in enumerate(self.particles):
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    aa *= emissionModels[i][distance]
            allPossible[particle] += aa
        ...
        if not any(allPossible.values()):
            ...
            for _ in range(0, self.numParticles):
                parti.append(util.sample(allPossible))
            self.particles = parti
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                ghost_position= setGhostPositions(gameState, newParticle)
                new_posi_dist = getPositionDistributionForGhost(ghost_position, i, self.ghostAgents[i])
                newParticle[i] = util.sample(new_posi_dist)
            "*** END YOUR CODE HERE ***"
